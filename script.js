...
function playerMovement() {
  ...
  if (player.collide(groundLine) || player.collide(block)) {
     ...
     if (keyDown("up_arrow")) {
        playSound("sound://category_jump/ninja_jump_2.mp3", false);
...
function playerObjects() {
  // JUMP RINGS
    ...
    if (jumpRings.isTouching(player) && keyWentDown("up")) {
      playSound("sound://category_points/vibrant_game_ding_touch_1.mp3", false);
...
if (jumpPads.isTouching(player)) {
   playSound("sound://category_jump/arcade_game_jump_15.mp3", false);
   if (currentSize == 0) {
    effectCircle2.tint = "rgb(245, 255, 20)";
    effectCircle2.scale = 2;
    if (currentGravity == 0) {
...
function playerGravityPortals() {
  ...
  if (bluePortals.isTouching(player) && currentGravity == 1) {
   playSound("sound://category_poof/puzzle_game_poof_bubble_big_02.mp3", false);
   if (player.velocityY < -50) {
     player.velocityY = -50;
   }
...
function playerRespawn() {
  ...
  // Death sound
  if (respawnTimer == 35) {
    playSound("sound://category_poof/puzzle_game_poof_b_02.mp3");
  }
